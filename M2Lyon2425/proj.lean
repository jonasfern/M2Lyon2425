...
theorem card_so2_char22 (h : ringChar F = 2) : (Nat.card SO(2,F) = Nat.card F) := by
have q := Nat.card F
refine Nat.card_eq_of_bijective f ?_
...
intro A
  ...
  have ha := SO2_inv_co A
  have hb := SO2_inv_co B
  have h1:= ha.right.right
  have c:= A.1 0 0
  rw[← frobenius_def F (Fin 2)] at h1
  rw[frobenius F 2] at h1
  match i, j with
  |0, 0 => exact h
  |0,1 =>
  |1, 0 => sorry
  |1,1 =>  rw[ha.right.left, hb.right.left] at h; exact h
